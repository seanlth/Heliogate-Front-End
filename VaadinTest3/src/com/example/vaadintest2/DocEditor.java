...
public class DocEditor extends CustomComponent {
	...
	public DocEditor() {
		...
		for (int i = 0; i < names.length; i++) {
			comboBox_3.addItem(names[i]);
		}
		comboBox_3.setValue(comboBox_3.getItemIds().iterator().next());
		
		comboBox_6.setNullSelectionAllowed(false);
		comboBox_6.setInvalidAllowed(false);
		comboBox_6.addItem("Some workflow");
		comboBox_6.setValue(comboBox_6.getItemIds().iterator().next());
	...
	private String[] queryCatalogueList() throws Exception
	{
		String catalogueNames[] = new String[79]; //just hard coded for now
        ...
        while ((inputLine = in.readLine()) != null) {
        	if (inputLine.contains("\"Catalogue\"") ) {
        		if (line > 0) {
        			catalogueNames[line-1] = "";
        			int name = inputLine.indexOf("\"Catalogue\"");
        			for (int i = name+12; inputLine.charAt(i) != '<'; i++) {
        				catalogueNames[line-1] += inputLine.charAt(i); 
        			}
        		}
        		line++;
        		if (line > 79) {
	...
	private AbsoluteLayout buildTop_sean_area() {
		...
		top_sean_area.addComponent(button_3, "top:179.0px;left:607.0px;");
